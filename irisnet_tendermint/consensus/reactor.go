...
func (conR *ConsensusReactor) Receive(chID byte, src p2p.Peer, msgBytes []byte) {
	fmt.Println("Got something on consensus reactore recieve()")
	if !conR.IsRunning() {
		conR.Logger.Debug("Receive", "src", src, "chId", chID, "bytes", msgBytes)
		return
	}

	// Marlin Peer-sending to tendermint if received from marlin peer
	...
	// Marlin Peer-sending everything on marlin peer
